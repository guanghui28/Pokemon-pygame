...
class AllSprites(pygame.sprite.Group):
    ...
    def custom_draw(self, player):
        ...
        for layer in (bg_sprites, main_sprites, fg_sprites):
            for sprite in layer:
                rect_pos = sprite.rect.topleft + self.offset
                if isinstance(sprite, Entity):
                    self.display_surface.blit(self.shadow_surf, rect_pos + vector(40, 110))
                self.display_surface.blit(sprite.image, rect_pos)
...
class BattleSprites(pygame.sprite.Group):
    ...
    def custom_draw(self, current_monster_sprite, side, mode, target_index, player_sprites, opponent_sprites):
        ...
        for sprite in sorted(self, key=lambda sprite: sprite.z):
            if sprite.z == BATTLE_LAYERS['outline']:
                if sprite.monster_sprite == current_monster_sprite and not (mode == 'target' and side == 'player') or sprite.monster_sprite == monster_sprite and \
                    sprite.monster_sprite.entity == side and \
                    mode and mode == 'target':
